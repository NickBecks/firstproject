...
function enableButtons () {
	...
	// Update the button label now that the button is active
	btn.value="Click me for a periodical greet";
	...
	btn = document.getElementById("input_greet_nicely");
	btn.onclick=function(){greetNicely();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a nice greet";
}
...
function greetNicely(){
	var name = document.getElementById("name_field").value;
	var period = document.getElementById("period_field").value; 
	var compliment = document.getElementById("nice_field").value;
	var request = gapi.client.helloworldendpoints.greetNicely({'name': name, 'period':period, 'niceWords':compliment});
	request.execute(sayHelloCallback);
	}
